...
Shader "Unlit/SpecialFX/Liquid" {
    ...
    SubShader {
        ...
        Pass {
            ...
            sampler2D _MainTex;
            float4 _MainTex_ST, _WorldPos, _ObjectSize;
            float _FillAmount, _Adjustment, _WobbleX, _WobbleZ;
            float4 _TopColor, _RimColor, _FoamColor, _Tint;
            float _Rim, _RimPower, _Max, _Min;
            ...
            v2f vert(appdata v)
            {
                ...
                float3 worldPosAdjusted = worldPos.xyz + (worldPosX * _WobbleX) + (worldPosZ * _WobbleZ);
                // how high up the liquid is
                float fill =  Remap(_FillAmount, 0, 1, _Min, _Max);
                o.fillEdge = worldPos.y - fill;
